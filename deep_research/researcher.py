    ...
    def fetch(
        ...
        *,
        ...
        with self._semaphore:
            return fetcher.fetch(url, timeout_seconds=timeout_seconds)
...
class Researcher:
    ...
    def research(self, task: ResearchTask) -> ResearchResult:
        ...
        futures: list[Future[tuple[PageExploration, list[EvidenceObservation], str | None]]] = [
            executor.submit(self._process_page, task, url) for url in candidates
        ]
        try:
    ...
    def _generate_queries(self, task: ResearchTask) -> list[SearchQuery]:
    ...
    def _process_page(
        self, task: ResearchTask, url: str
        ...
        if not self.urls.claim_content(page.content_hash):
            self.audit.log("page.skipped_duplicate_content", exploration=exploration)
            return exploration, [], None
        try:
            extracted = self._extract_observations(task, page)
    ...
    def _extract_observations(self, task: ResearchTask, page) -> list[EvidenceObservation]:
        payload = self.model.generate_json(
            system_prompt=(
