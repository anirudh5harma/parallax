...
export default function Home() {
  ...
  const connect = useCallback((session: SessionSummary) => {
    ...
    const push = (data: { message?: string; stage?: string }, event: MessageEvent, tone?: Activity['tone']) => { if (data.message && activeIdRef.current === session.id) setActivity((items) => [...items, { id: `${session.id}-${event.lastEventId}`, message: data.message!, stage: data.stage, tone }]); };
    ...
    stream.addEventListener('report.chunk', (raw) => { const data = consume(raw as MessageEvent); if (data?.text && activeIdRef.current === session.id) setStreamedReport((value) => value + data.text); });
  ...
  useEffect(() => { Promise.all([api.health(), api.sessions()]).then(([health, items]) => { setConfigured(health.configured); setModel(health.model.replace('us.anthropic.claude-', 'Opus ').replace('-v1', '').replaceAll('-', '.')); setSessions(items); if (items[0]) { activeIdRef.current = items[0].id; setActiveId(items[0].id); connect(items[0]); } }).catch((cause: Error) => setError(`Research service unavailable: ${cause.message}`)); return () => streamRef.current?.close(); }, [connect]);

  function selectSession(session: SessionSummary) { if (session.id === activeIdRef.current) return; activeIdRef.current = session.id; seenEventsRef.current.forEach((key) => { if (key.startsWith(`${session.id}:`)) seenEventsRef.current.delete(key); }); setDetail(null); setStreamedReport(''); setActivity([]); setError(null); setActiveId(session.id); connect(session); }
  ...
  function newResearch() { streamRef.current?.close(); activeIdRef.current = null; setActiveId(null); setDetail(null); setStreamedReport(''); setActivity([]); setError(null); setTab('report'); }
  const running = active && !terminal.has(active.status); const contested = detail?.evidence.filter((claim) => claim.disagreement).length ?? 0;
  const allowedReportUrls = useMemo(() => new Set(detail?.evidence.flatMap((claim) => claim.observations.map((observation) => observation.source_url)) ?? []), [detail]);
