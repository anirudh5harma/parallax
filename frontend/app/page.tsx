...
function Welcome({ query, setQuery, submit, busy, configured, mode, setMode }: { query: string; setQuery: (value: string) => void; submit: (event: FormEvent<HTMLFormElement>) => void; busy: boolean; configured: boolean | null; mode: ResearchMode; setMode: (value: ResearchMode) => void }) {
  return <div className="welcome"><div className="welcome-mark"><span /><span /></div><h1>What should we research?</h1><p>Ask a complex question. Review the plan before any sources are searched.</p><form className="composer" onSubmit={submit}><textarea autoFocus onChange={(event) => setQuery(event.target.value)} placeholder="Ask a research question…" rows={4} value={query} /><div><div className="mode-picker" aria-label="Research depth"><button aria-pressed={mode === 'fast'} className={mode === 'fast' ? 'active' : ''} onClick={() => setMode('fast')} type="button"><strong>Fast</strong><small>Broad · quicker</small></button><button aria-pressed={mode === 'deep'} className={mode === 'deep' ? 'active' : ''} onClick={() => setMode('deep')} type="button"><strong>Deep</strong><small>More sources · longer</small></button></div><button className="create-plan" disabled={!query.trim() || busy || configured === false} type="submit">{busy ? 'Planning…' : 'Create plan'} <i aria-hidden="true">↑</i></button></div></form><div className="examples">{examples.map((example) => <button key={example} onClick={() => setQuery(example)} type="button">{example}</button>)}</div></div>;
}
