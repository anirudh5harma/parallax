...
import { ApiError, EvidenceClaim, Observation, ResearchEventStream, ResearchMode, SessionDetail, SessionStatus, SessionSummary, api } from '../lib/api';
...
export default function Home() {
  ...
  async function createPlan() {
    ...
    try { const session = await api.create(query.trim(), mode); setSessions((items) => [session, ...items]); setQuery(''); selectSession(session); }
...
function Welcome({ query, setQuery, submit, busy, configured, mode, setMode }: { query: string; setQuery: (value: string) => void; submit: (event: FormEvent<HTMLFormElement>) => void; busy: boolean; configured: boolean | null; mode: ResearchMode; setMode: (value: ResearchMode) => void }) {
  return <div className="welcome"><div className="welcome-mark"><span /><span /></div><h1>What should we research?</h1><p>Ask a complex question. Review the plan before any sources are searched.</p><form className="composer" onSubmit={submit}><textarea autoFocus onChange={(event) => setQuery(event.target.value)} placeholder="Ask a research question…" rows={4} value={query} /><div><label className="mode-select"><span aria-hidden="true">⌁</span><div><strong>{mode === 'fast' ? 'Fast research' : 'Deep research'}</strong><small>{mode === 'fast' ? 'Broad · quicker' : 'Maximum breadth · longer'}</small></div><select aria-label="Research mode" onChange={(event) => setMode(event.target.value as ResearchMode)} value={mode}><option value="fast">Fast research</option><option value="deep">Deep research</option></select><i aria-hidden="true">⌄</i></label><button className="create-plan" disabled={!query.trim() || busy || configured === false} type="submit">{busy ? 'Planning…' : 'Create plan'} <i aria-hidden="true">↑</i></button></div></form><div className="examples">{examples.map((example) => <button key={example} onClick={() => setQuery(example)} type="button">{example}</button>)}</div></div>;
}
...
function PlanReview({ session, busy, start }: { session: SessionSummary; busy: boolean; start: () => void }) {
  const { plan, budget_limits: budget, mode } = session;
  return <div className="assistant-message plan-message"><span className="assistant-mark">P</span><div><p className="assistant-intro">I’ll investigate four evidence paths in parallel and scan broadly across the web. Review the scope, then start research.</p><section className="plan-card"><div className="plan-card-head"><div><span>{mode} plan</span><strong>{plan.length} steps</strong></div><span className="budget-note">Up to {budget.max_sources} screened · {budget.max_pages} read</span></div><ol>{plan.map((task, index) => <li key={task.id} style={{ '--delay': `${index * 70}ms` } as React.CSSProperties}><span>{index + 1}</span><div><strong>{task.question}</strong><p>{task.rationale}</p></div><small>{task.priority}</small></li>)}</ol><div className="plan-actions"><p>No web research begins before approval.</p><button disabled={busy} onClick={start} type="button">{busy ? 'Starting…' : `Start ${mode} research`} <span aria-hidden="true">→</span></button></div></section></div></div>;
}
